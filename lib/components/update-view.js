...
export default class UpdateView extends EtchComponent {
  ...
  handleAutoUpdateCheckbox (e) {
    soldat.config.set('core.automaticallyUpdate', e.target.checked)
  }
      ...
      case UpdateManager.State.Idle:
        updateStatus = (
          <div className='about-updates-item is-shown about-default-update-message'>
            {this.props.updateManager.getAutoUpdatesEnabled() ? 'Soldat will check for updates automatically' : 'Automatic updates are disabled please check manually'}
          </div>
        )
        break
      ...
      case UpdateManager.State.UpToDate:
        updateStatus = (
          <div className='about-updates-item app-up-to-date'>
            ...
            <span className='about-updates-label is-strong'>Soldat is up to date!</span>
          </div>
        )
        break
